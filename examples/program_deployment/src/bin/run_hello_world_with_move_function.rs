...
use lee::{Position, PublicTransaction, program::Program, public_transaction};
    ...
    match cli.command {
        Command::WritePublic {
            ...
            let message = public_transaction::Message::try_new(
                program.id().into(),
                vec![Position::new(account_id, program.id().into())],
                nonces,
                instruction,
            )
            .unwrap();
        ...
        Command::WritePrivate {
            ...
            let account_id = account_id.parse().unwrap();
            let accounts =
                vec![AccountIdentity::PrivateOwned(account_id).in_namespace(program.id().into())];
        ...
        Command::MoveDataPublicToPublic { from, to } => {
            ...
            let message = public_transaction::Message::try_new(
                program.id().into(),
                vec![
                    Position::new(from, program.id().into()),
                    Position::new(to, program.id().into()),
                ],
                nonces,
                instruction,
            )
        ...
        Command::MoveDataPublicToPrivate { from, to } => {
            ...
            let accounts = vec![
                AccountIdentity::Public(from).in_namespace(program_id),
                AccountIdentity::PrivateOwned(to).in_namespace(program_id),
            ];

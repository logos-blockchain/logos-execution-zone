...
type Instruction = (u128, u32, Option<PdaSeed>);

/// A program that calls the authenticated transfers program `num_chain_calls` times, permuting
/// the order of the input accounts on the subsequent call.
fn main() {
    ...
    let ProgramCall::Execute(
        ProgramInput {
            ...
            caller_account_id,
            pre_states,
            instruction: (balance, num_chain_calls, pda_seed),
        },
        instruction_data,
    ) = call
    else {
    ...
    for _i in 0..num_chain_calls {
        let new_chained_call = ChainedCall {
            program_account_id: authenticated_transfer_core::authenticated_transfer_account_id(),

...
/// The test block-producer's reward account as a genesis-seed entry, for stores that seed
/// their state before any block credits fees to it.
#[must_use]
...
pub fn create_transaction_native_token_transfer_with_fees(
    ...
    let positions = vec![Position::balance_only(from), Position::balance_only(to)];

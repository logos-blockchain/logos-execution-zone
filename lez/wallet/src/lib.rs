...
impl WalletCore {
    ...
    /// Submits an already-built public transaction. For the common case of building one from a
    /// set of `AccountIdentity`s, prefer [`Self::send_pub_tx`]; this is the primitive underneath
    /// it, exposed for callers that must construct the message/witness set themselves — e.g. to
    /// add a fee payer that signs without being one of the instruction's own accounts (see
    /// `program_facades::program_loader`, whose claimed accounts hold nothing to self-pay with).

...
pub enum ProgramLoaderSubcommand {
    ...
    UploadHeader {
        ...
        #[arg(long)]
        immutable: bool,
        /// An existing, funded account to pay the fee from. See `NewSegment`'s `payer`.
        #[arg(long)]
        payer: Option<CliAccountMention>,
    },
    ...
    UpdateHeader {
        ...
        /// been funded either — see `NewSegment`'s `payer`.
        #[arg(long)]
        payer: Option<CliAccountMention>,
    },
...
impl ProgramLoaderSubcommand {
    async fn handle_new_segment(
        ...
        let tx_hash = ProgramLoader(wallet_core)
            .new_segment(target_id, bytecode, next_segment, payer_id)
            .await?;
    ...
    async fn handle_subcommand(
        ...
        match self {
            Self::NewSegment {
                ...
                Self::handle_new_segment(target, bytecode_file, next_segment, payer, wallet_core)
                    .await
            }
            Self::UploadHeader {
                target,

...
pub enum ProgramLoaderSubcommand {
    ...
    WriteSegment {
        ...
        /// An existing, funded account to pay the fee from. `target` is always a fresh deploy
        /// account and so holds nothing to self-pay with; omit to fall back to the wallet's
        /// ordinary self-pay selection (which will fail unless `target` is otherwise funded).
        #[arg(long)]

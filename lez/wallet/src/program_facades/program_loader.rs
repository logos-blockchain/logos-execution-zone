...
impl ProgramLoader<'_> {
    ...
    /// `accounts` — is never added to the message's `account_ids`, since `program_loader`'s guest
    /// code asserts an exact account count/shape per instruction and an extra trailing account
    ...
    /// segment — chains are always linked tail-to-head. See [`Self::send`] for `payer`.
    ...
    async fn upload_segments_and_header(
        ...
        for i in (0..chunks.len()).rev() {
            ...
            let tx_hash = self
                .new_segment(segments[i], chunks[i].to_vec(), next_segment, payer)
                .await

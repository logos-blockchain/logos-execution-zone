...
impl Ata<'_> {
    pub async fn send_create(
        ...
        let ata_program_id = AccountId::builtin_default_address(programs::ata().id());
...
fn ata_with_token_dependency() -> ProgramWithDependencies {
    ...
    deps.insert(AccountId::builtin_default_address(token.id()), token);
    let ata = programs::ata();
    let ata_id = AccountId::builtin_default_address(ata.id());

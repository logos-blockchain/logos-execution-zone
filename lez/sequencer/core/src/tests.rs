...
fn only_the_cross_zone_inbox_is_sequencer_only() {
    ...
    assert!(!is_sequencer_only_program(
        loader_core::immutable_deploy_account_id(programs::wrapped_token().id())
    ));
    assert!(!is_sequencer_only_program(
        loader_core::immutable_deploy_account_id(programs::ping_sender().id())
    ));
    assert!(!is_sequencer_only_program(
        loader_core::immutable_deploy_account_id(programs::clock().id())
    ));
}
...
/// Builds a single-transaction Deploy covering the whole of `bytecode` — matches `-7`'s
/// behavior: `first_segment: 0`, declared `image_id`/`segment_count` derived honestly from the
/// real bytes, no signer required (verified immediately against the real bytes by
/// `execute_deploy`, not trusted).
fn deploy_transaction(
    ...
    let image_id = loader_core::compute_image_id(&user_elf).unwrap_or([0; 8]);
...
fn deploy_transactions(
    ...
    let user_elf = loader_core::extract_user_elf(bytecode).unwrap();
    let image_id = loader_core::compute_image_id(&user_elf).unwrap();
    let segment_count = loader_core::segment_count_for(&user_elf);
    assert_eq!(
        ...
        let batch_segments: Vec<AccountId> = (0..batch_len)
            .map(|i| {
                let segment_number = first_segment
                    ...
                    .unwrap();
                loader_core::deploy_segment_account_id(
                    loader_id,
                    image_id,
                    segment_number,
                    update_auth,
            ...
            .collect();
        let header = loader_core::deploy_header_account_id(loader_id, image_id, 0, update_auth);
        let batch_byte_len = batch_len
            .checked_mul(loader_core::MAX_SEGMENT_DATA_LEN)
            .unwrap()
...
fn loader_deploys_program_across_multiple_transactions() {
    ...
    let segment_ids: Vec<AccountId> = (0..segment_count)
        .map(|i| loader_core::deploy_segment_account_id(loader_id, image_id, i, update_auth))
        .collect();
    ...
    let deployed_header = state.get_account_by_id(header);
    let program_data = loader_core::ProgramData::try_from(&deployed_header.data).unwrap();
...
fn loader_deploy_batches_can_land_out_of_order() {
    ...
    let tail_offset = loader_core::MAX_SEGMENT_DATA_LEN.min(user_elf.len());
...
fn loader_get_program_returns_none_for_abandoned_multi_tx_deploy() {
    ...
    let header = {
        let loader_id: ProgramId = RESERVED_DEPLOYMENT_PROGRAM_ACCOUNT_ID.into();
        let image_id = loader_core::compute_image_id(&user_elf).unwrap();
        loader_core::deploy_header_account_id(loader_id, image_id, 0, update_auth)
    };
...
fn loader_rejects_a_partial_deploy_batch_with_default_update_auth() {
    ...
    let header = loader_core::deploy_header_account_id(loader_id, image_id, 0, update_auth);
    let segment0 = loader_core::deploy_segment_account_id(loader_id, image_id, 0, update_auth);
    ...
    let tx = deploy_batch_transaction(
        header,
        &[segment0],
        &user_elf[..loader_core::MAX_SEGMENT_DATA_LEN.min(user_elf.len())],
        image_id,
        segment_count,
        0,
        update_auth,
...
fn loader_rejects_header_racing_with_a_different_segment_count() {
    ...
    let header = loader_core::deploy_header_account_id(loader_id, image_id, 0, update_auth);
    let segment0 = loader_core::deploy_segment_account_id(loader_id, image_id, 0, update_auth);
    let segment1 = loader_core::deploy_segment_account_id(loader_id, image_id, 1, update_auth);
...
fn loader_rejects_overlapping_segment_rewrite() {
    ...
    let batch_bytes = &user_elf[..loader_core::MAX_SEGMENT_DATA_LEN.min(user_elf.len())];
    let tx1 = deploy_batch_transaction(
        header,
        &[segment0],
        batch_bytes,

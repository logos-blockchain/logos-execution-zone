...
use sequencer_bedrock_actor::{
    BedrockActorTrait,
    protocol::{Ed25519Key, LiveChannelConfig},
};
...
impl<S: StorageActorTrait, B: BedrockActorTrait> SequencerCore<S, B> {
    ...
    pub async fn on_channel_update(
        ...
        let (resubmit_txs, outcome, head_height) = {
            ...
            let mut to_persist: Vec<Block> = vec![];
            let mut events = vec![];

            for (block, adopted_outcome) in adopted
                .iter()
                .zip(&outcomes)
                .filter(|(_, outcome)| matches!(outcome, AcceptOutcome::Applied(_)))
            {
                to_persist.push(block.clone());
                if let AcceptOutcome::Applied(block_events) = adopted_outcome {
                    events.extend(block_events.clone());
                }
            }
            ...
            for ((block, _), outcome) in finalized.iter().zip(finalized_outcomes) {
                match outcome {
                    AcceptOutcome::Applied(block_events) => {
                        to_persist.push(block.clone());
...
fn dispatch_already_delivered(state: &lee::V03State, message: &CrossZoneMessage) -> bool {
    let inbox_program_id = AccountId::from_builtin_program(programs::cross_zone_inbox().id());

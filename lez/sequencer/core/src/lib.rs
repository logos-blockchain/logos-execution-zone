...
use lee::{AccountId, Position, PublicTransaction, public_transaction::Message};
...
impl<S: StorageActorTrait, BP: BlockPublisherTrait> SequencerCore<S, BP> {
    ...
    async fn build_block_from_mempool(
        ...
        // startup replay.
        //
        // Skip any deposit whose receipt carries the bridge's record in the state we build on —
        // it was minted by us or by a peer whose block we adopted. Only the bridge can write
        // that shard, so nothing else can make a deposit look minted. An orphan reverts the
        // receipt with the block, so the next turn re-mints without any bookkeeping of our own.
...
/// Whether `deposit_op_id`'s mint is already reflected in `state` — a record in
/// the bridge's own shard, which only a mint can write. The receipt is the
/// exactly-once ledger the bridge program keeps.
fn deposit_already_minted(state: &lee::V03State, deposit_op_id: HashType) -> bool {
    ...
    state
        ...
        .is_some_and(|receipt| !receipt.shard(bridge_program_id).is_empty())
}
...
fn dispatch_already_delivered(state: &lee::V03State, message: &CrossZoneMessage) -> bool {
    ...
    state.get_account_by_id_ref(shard_id).is_some_and(|shard| {
        cross_zone_inbox_core::SeenShard::from_bytes(shard.shard(inbox_program_id).as_ref())
            .is_ok_and(|seen| {
...
fn genesis_stake_message(
    ...
    Message::try_new(
        ...
        vec![
            Position::balance_only(genesis_stake_funding_account()),
            Position::new(ownership_id, sequencer_stake_program_id),
            Position::balance_only(system_accounts::stake_funds_account_id(&ownership_id)),
            Position::new(
...
fn build_stake_genesis_transactions(
    ...
    let fund_message = Message::try_new(
        ...
        vec![
            Position::balance_only(system_accounts::faucet_account_id()),
            Position::balance_only(genesis_stake_funding_account()),
        ],
...
fn build_bridge_deposit_tx_from_event(event: &PendingDepositEventRecord) -> Result<LeeTransaction> {
    ...
    let message = Message::try_new(
        bridge_program_id,
        vec![
            Position::balance_only(system_accounts::bridge_account_id()),
            Position::balance_only(metadata.recipient_id),
            Position::new(receipt_id, bridge_program_id),
        ],
        Vec::new(),
...
fn build_finalize_unstake_tx(
    ...
    let message = Message::try_new(
        ...
        vec![
            Position::new(ownership_id, sequencer_stake_program_id),
            Position::balance_only(system_accounts::stake_funds_account_id(&ownership_id)),
            Position::balance_only(pending.destination),
            Position::new(
                system_accounts::sequencer_stake_config_account_id(),
                sequencer_stake_program_id,
            ),

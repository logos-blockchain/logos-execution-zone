...
//!
//! For `stake`, the funding account moves balance only (the mover chained call
//! reads only its signature, no program's record there), so it just needs to
//! sign for itself and hold at least `amount`. The ownership account should be
//! fresh (or already backing the same sequencer key). Both must already exist
//! in the wallet (e.g. via `wallet account new public`).
                ...
                .send_pub_tx(
                    vec![
                        ...
                        AccountIdentity::Public(ownership_account)
                            .in_namespace(sequencer_stake_program_id),

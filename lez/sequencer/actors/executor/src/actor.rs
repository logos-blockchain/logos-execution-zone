...
use sequencer_core::{
    ChannelConfigActor, MsgId, PinBehindTip, SequencerCore, SubmitConfig, TransactionOrigin,
    config::SequencerConfig, gossip::AccreditedKeysReceiver, task_group::TaskGroup,
};
...
use crate::{
    ...
    protocol::{
        ...
        GetLastBlockId, GetProofsAndRoot, GetStatus, GetStatusReply, GetTransaction, ProduceBlock,
...
impl<S: StorageActorTrait, BP: BedrockActorTrait> Message<GetStatus> for ExecutorActor<S, BP> {
    ...
    async fn handle(
        ...
        let (stall_reason, chain_height) = {
            let chain_state = self.sequencer.chain_state().lock().await;

            (
                chain_state.final_stall().cloned(),
                chain_state.head_tip().map_or(0, |tip| tip.block_id),
            )
        };
        ...
        Ok(GetStatusReply {
            chain_height,
            failed_attempts,
            blocked_attempts_count: self.blocked_attempts.count,
            blocked_attempts_behind: self.blocked_attempts.behind.map(|msg| *msg.as_ref()),
            stall_reason,
        })
    }
}

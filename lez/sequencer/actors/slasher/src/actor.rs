...
use crate::{
    ...
    error::Error,
    protocol::{
        Approval, Offence, Propose, Report, ReportedOffence, SetApprovalPublisher, SetCommittee,
    },
};
...
pub struct SlasherActor<S: StorageActorTrait = StorageActor> {
    ...
    /// The channel every approval is signed over.
    channel_id: [u8; 32],
...
impl<S: StorageActorTrait> Message<SetCommittee> for SlasherActor<S> {
    type Reply = ();
...
mod tests {
    ...
    /// Loaded with the whole committee accredited; a narrower `Propose` replaces it.
    async fn slasher(approver: Ed25519Key) -> Slasher {
        let own = SequencerKey::new(approver.public_key().to_bytes()).expect("valid key");
        let config = config_staking(&[own, offender(), peer(), second_peer()]);
        SlasherActor::spawn(SlasherActor::load(storage(), approver, config, CHANNEL).await)
    }
    ...
    async fn refresh(slasher: &Slasher, config: SequencerStakeConfig) {
        slasher
            .tell(SetCommittee(config))
            .await
            .expect("the slasher should accept a committee");
    }
    ...
    /// A node that never takes a turn never proposes, so the follow path is the
    /// only thing that moves its committee.
    #[tokio::test]
    async fn a_committee_refresh_admits_a_peer_the_loaded_config_turned_away() {
        let slasher = slasher(approver_signing_key()).await;
        report(&slasher, vec![signed_by_offender(INSCRIPTION)]).await;

        // A committee without the peer, so its approval is turned away.
        refresh(&slasher, config_staking(&[approver(), offender()])).await;
        send(&slasher, peer_approval(PEER_SECRET, INSCRIPTION)).await;
        assert!(propose(&slasher, committee()).await.is_empty());

        // The peer staked in a block this node followed, so its next approval counts.
        refresh(&slasher, committee()).await;
        send(&slasher, peer_approval(PEER_SECRET, INSCRIPTION)).await;

...
///
/// Combines the program's content-derived identity (`image_id`), `segment_number` (always `0`
...
/// [`deploy_segment_pda_seed`]), and `update_auth` — included so multiple independent
/// deployments of identical bytecode land at distinct accounts instead of colliding.
///
...
pub fn plan_deploy_range(
    ...
    let header = PlannedAccount {
        account_id: AccountId::for_public_pda(&loader_program_id, &header_seed),
        seed: header_seed,
    ...
    let segments = chunks
        ...
        .map(|(i, chunk)| {
            ...
            PlannedAccount {
                account_id: AccountId::for_public_pda(&loader_program_id, &seed),
                seed,
                data: chunk.to_vec(),
            }
        })
...
///   declared one right here — exactly as trustworthy as a `-7`-style single-transaction deploy,
...
pub fn execute_deploy(
    ...
    check_header_target(self_program_id, header_target, &plan.header);
...
fn check_header_target(
    ...
    let already_this_header = Account {
        program_owner: AccountId::from(self_program_id),
        data: Data::try_from(planned.data.clone())
            .expect("elf chunk must fit under DATA_MAX_LENGTH"),
        ..Account::default()
    };
...
mod tests {
    ...
    const LOADER_ID: ProgramId = [9, 10, 11, 12, 13, 14, 15, 16];
    ...
    fn execute_deploy_two_batch_sequence_succeeds() {
        ...
        let header_after_batch1 = Account {
            program_owner: AccountId::from(LOADER_ID),

...
fn init_config(
    ...
    // a written one must already hold exactly this minter (the genesis block is
    // replayed onto seeded state during multi-sequencer reconstruction), otherwise
    // reject a post-genesis attempt to set a different minter.

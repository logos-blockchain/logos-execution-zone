...
use lee_core::{
    account::AccountId,
    program::{
        AccountMeta, LeeCall, Plan, ProgramInput, ResolveInput, read_lee_call, resolve_write,
    },
};
...
enum Effect {
    // The whole backing for inbound cross-zone value. Wrapped tokens have no reserve or
    // supply check anywhere else, so a gap here is unbacked money creation, not a bookkeeping
    // slip.
    Mint {
...
fn main() {
    match read_lee_call::<Instruction>() {
        ...
        LeeCall::Resolve(input) => {
            let data = resolve(&input);
            resolve_write(
                input,
                data.try_into()
                    .expect("wrapped-token data fits in account data"),
            )
        }
    }
}

// Every effect writes, so this program never keeps a shard: an inspection it could only keep
// would be an inspection of someone else's data.
fn resolve(input: &ResolveInput) -> Vec<u8> {
    // The foreign-shard write rejection would catch a misdirected effect too, but only after
    // the mint had been authorized against data whoever owns that shard wrote.
    assert_eq!(
        input.selector.program_account_id, input.self_account_id,
        "wrapped_token only resolves effects on its own shard"
    );
    let effect: Effect =
        borsh::from_slice(&input.effect_data).expect("wrapped_token wrote its own effect");
    ...
    match effect {
        Effect::Mint {
            ...
            marker,
            amount,
        } => mint_source(&input.pre_data, caller_account_id, marker, amount),
        ...
        // refused, because the genesis block is replayed onto seeded state during
        // multi-sequencer reconstruction.
        Effect::InitConfig(config_value) => {
            let bytes = config_value.to_bytes();
...
fn execute(input: &ProgramInput<Instruction>, instruction_data: Vec<u8>) -> Plan {
    match &input.instruction {
        Instruction::Mint { recipient, amount } => {
            mint(input, instruction_data, recipient, *amount)
        }
        Instruction::InitConfig(config) => init_config(input, instruction_data, config),
        Instruction::RenounceAuthority => renounce_authority(input, instruction_data),
        Instruction::UpdateSources { sources } => update_sources(input, instruction_data, sources),
    }
}
...
fn mint(
    ...
    plan.update(holding, &Effect::Credit(amount));
    plan
}
...
fn renounce_authority(input: &ProgramInput<Instruction>, instruction_data: Vec<u8>) -> Plan {
...
// who the authority is stay in the config's resolver.
fn governance_accounts(input: &ProgramInput<Instruction>) -> (&AccountMeta, &AccountMeta) {
...
mod tests {
    ...
    fn resolve_on(program_account_id: AccountId, pre_data: ShardData, effect: &Effect) -> Vec<u8> {
        resolve(&ResolveInput {
            self_account_id: WRAPPED_ID,
            selector: ProgramShardSelector::new(config_account_id(WRAPPED_ID), program_account_id),
            pre_data,
            effect_data: borsh::to_vec(effect).expect("the effect serializes"),
        })
    }

    fn resolve_at(pre_data: ShardData, effect: &Effect) -> Vec<u8> {
        resolve_on(WRAPPED_ID, pre_data, effect)
    }
    ...
    fn resolved_config(pre_data: ShardData, effect: &Effect) -> WrappedTokenConfig {
        WrappedTokenConfig::from_bytes(&resolve_at(pre_data, effect)).expect("a config was written")
    }

    fn mint_accounts(marker: AccountId) -> Vec<AccountMeta> {
        vec![
            AccountMeta::balance(marker, false),
    ...
    fn governance_metas(authority: AccountId, is_authorized: bool) -> Vec<AccountMeta> {
        vec![
            AccountMeta::new(config_account_id(WRAPPED_ID), false, WRAPPED_ID),
            AccountMeta::balance(authority, is_authorized),
        ]
    }
    ...
    fn a_credit_adds_to_the_recipients_balance() {
        assert_eq!(
            resolve_at(shard(balance_bytes(40).to_vec()), &Effect::Credit(2)),
            balance_bytes(42).to_vec()
        );
        assert_eq!(
            resolve_at(ShardData::empty(), &Effect::Credit(42)),
            balance_bytes(42).to_vec()
        );
    }
    ...
    fn a_credit_that_overflows_the_holding_is_refused() {
        resolve_at(shard(balance_bytes(u128::MAX).to_vec()), &Effect::Credit(1));
    }
    ...
    fn a_first_init_writes_the_config_and_a_replay_is_a_no_op() {
        assert_eq!(
            resolve_at(ShardData::empty(), &Effect::InitConfig(config())),
            config().to_bytes()
        );
        assert_eq!(
            resolve_at(shard(config().to_bytes()), &Effect::InitConfig(config())),
            config().to_bytes()
        );
    }

    #[test]
    #[should_panic(expected = "wrapped-token config already initialized differently")]
    ...
    #[test]
    #[should_panic(expected = "wrapped_token only resolves effects on its own shard")]
    fn an_effect_resolved_against_a_foreign_shard_is_refused() {
        // These bytes would authorize the mint. They are refused because the handle names a
        // shard this program does not own, and whoever does own it chose its contents.
        resolve_on(
            STRANGER,
            shard(config().to_bytes()),
            &mint_effect(Some(MINTER), marker_of(MINTER, ZONE_A, PEER_A), 1),
        );
    }
}

...
impl AccountsForExeTests {
    fn user_token_a_holding() -> Account {
        Account {
            program_owner: AccountId::from_builtin_program(programs::token().id()),
            balance: 0_u128,
            data: Data::from(&TokenHolding::Fungible {
    ...
    fn pool_definition_init() -> Account {
        Account {
            program_owner: AccountId::from_builtin_program(programs::amm().id()),
            balance: 0_u128,
            data: Data::from(&PoolDefinition {
...
fn simple_amm_new_definition_inactive_initialized_pool_and_uninit_user_lp() {
    ...
    let instruction = amm_core::Instruction::NewDefinition {
        ...
        amm_program_id: AccountId::from_builtin_program(programs::amm().id()),
    };

...
use lee_core::account::{AccountId, BalanceDiff, Input};

use super::*;

fn empty_target(account_id: AccountId, is_authorized: bool) -> Input {
    Input::named(
        account_id,
        is_authorized,
        0,
...
fn segment_pre(account_id: AccountId, bytecode: Vec<u8>, next_segment: Option<AccountId>) -> Input {
    Input::named(
        account_id,
        false,
        0,
...
fn header_pre(account_id: AccountId, header: &ProgramHeader, is_authorized: bool) -> Input {
    Input::named(
        account_id,
        is_authorized,
        0,
...
fn write_segment_writes_the_loader_shard() {
    ...
    assert_eq!(diffs[0].pre.account_id, target_id);
...
fn write_segment_accepts_a_target_holding_balance() {
    ...
    assert_eq!(diffs.len(), 1);
    assert_eq!(diffs[0].pre.balance, 5);
    assert_eq!(diffs[0].post_balance_diff, BalanceDiff::Add(0));
}
...
fn write_segment_linking_to_an_existing_segment_leaves_it_unchanged() {
    ...
    assert_eq!(diffs[1].pre, next_pre);
...
fn write_segment_rejects_a_next_segment_position_naming_another_namespace() {
    let target_id = AccountId::new([1; 32]);
    let next_id = AccountId::new([2; 32]);
    let foreign_next = Input::named(
        next_id,
        false,
        0,
        AccountId::new([9; 32]),
...
fn write_segment_rejects_a_next_segment_with_malformed_data() {
    let target_id = AccountId::new([1; 32]);
    let next_id = AccountId::new([2; 32]);
    let malformed_next = Input::named(
        next_id,
        false,
        0,
        PROGRAM_LOADER_ACCOUNT_ID,

...
///
/// Same inputs (and the same genesis-not-current caveat) as [`deploy_header_pda_seed`],
...
pub fn deploy_header_account_id(
    ...
    AccountId::for_public_pda(
        &loader_account_id,
        &deploy_header_pda_seed(genesis_image_id, segment_number, genesis_update_auth),
    )
}
...
pub fn deploy_segment_account_id(
    ...
    AccountId::for_public_pda(
        &loader_account_id,
        &deploy_segment_pda_seed(genesis_image_id, segment_number, genesis_update_auth),
    )
}
...
pub fn plan_deploy_range(
    ...
    let header_seed = deploy_header_pda_seed(genesis_image_id, 0, genesis_update_auth);
...
pub fn finalize(
    ...
    for (segment_number, target) in segment_targets.iter().enumerate() {
        ...
        let expected_account_id = deploy_segment_account_id(
            loader_id,

...
use lee_core::{
    ...
    program::{AccountStateDiff, Claim, PROGRAM_LOADER_ACCOUNT_ID, ProgramId},
};
...
pub fn write_segment(
    ...
    let mut diffs = Vec::with_capacity(pre_states.len());
    diffs.push(AccountStateDiff::new_claimed(
        target.clone(),
        BalanceDiff::Add(0),
        Data::from(&ProgramSegment {
            bytecode,
            next_segment,
        }),
        Claim::Authorized,
...
pub fn create_header(
    ...
    let mut diffs = vec![AccountStateDiff::new_claimed(
        pre_states[0].clone(),
        BalanceDiff::Add(0),
        Data::from(&ProgramHeader {
            image_id,

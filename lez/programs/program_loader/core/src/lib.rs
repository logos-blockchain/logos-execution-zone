...
//!
//! Every instruction here writes only the loader's own shard, so an account's other namespaces
//! and its balance are untouched whoever else holds them. A fresh segment or header needs no
//! authorization because that shard is empty, while `UpdateHeader` rewrites an existing one and
//! needs `is_authorized` — checked here directly rather than through the diff-validation rules
//! (which only gate balance decreases on authorization).
...
use lee_core::{
    ...
    program::{PROGRAM_LOADER_ACCOUNT_ID, ProgramId, ShardStateDiff},
};
...
pub enum Instruction {
    /// Writes one new bytecode segment at `pre_states[0]` (its loader shard must be empty —
    /// write-once, no instruction edits a segment after creation). Permissionless: no
    /// authorization is required of `pre_states[0]`, since an empty shard is nobody's.
    ///
    ...
    /// Creates a new program header at `pre_states[0]` (its loader shard must be empty;
    ...
    /// Rewrites an existing header at `pre_states[0]` (its loader shard must already hold a valid
    /// [`ProgramHeader`], be `is_authorized`, and not already be `immutable`). Unlike the other
    /// two instructions this is an ordinary mutation of a shard the loader already wrote, so it
    /// needs the authorization the other two don't. Same chain/`image_id` handling as
    /// `CreateHeader`.
    ///
    /// Required accounts (1 + the segment chain length).
    UpdateHeader {
...
fn compute_image_id(segments_with_header: &[Input]) -> ProgramId {
    let mut elf = Vec::new();

...
fn record(
    ...
    // pre_states: [source marker (balance-only, only its address is read), config
    // PDA, record PDA].
    ...
    let post = ShardStateDiff::new(record, BalanceDiff::Add(0), record_data);
...
fn renounce_authority(
    ...
    // pre_states: [config PDA, authority (balance-only, it only signs)].
    ...
    ProgramOutput::new(
        ...
        instruction_data,
        vec![
            ShardStateDiff::new(config, BalanceDiff::Add(0), config_data),

...
///
/// The sequencer passes the current block timestamp together with the block ID the every-block
/// clock account advances to; that ID is a proposal, pinned by the every-block account's own
/// effect to its actual prior ID plus one.

...
mod tests {
    ...
    // Deploys the emitter guest through `program_loader`: one `WriteSegment` per
    // `program_loader_core::MAX_SEGMENT_DATA_LEN`-sized chunk of the ELF (linked tail-to-head,
    // the last chunk written first, each earlier one naming the already-written next segment),
    // then a `CreateHeader` claiming a fresh header account that points at the first one. All
    // land in the same block, in order, so each transaction sees the accounts the preceding ones
    // just committed. All are ordinary (non-exempt) public transactions, so each needs its own
    // fee declaration; a funded genesis account co-signs as payer since the freshly-claimed
    // segment/header accounts hold nothing to self-pay with.
    fn deploy_emitter_txs() -> Vec<LeeTransaction> {
        ...
        let chunks: Vec<&[u8]> = test_methods::EVENT_EMITTER_ELF
            .chunks(program_loader_core::MAX_SEGMENT_DATA_LEN)
            .collect();
        // Base 210 keeps these clear of `emitter_header_key`'s [201; 32] — a flat-byte key this
        // close by would otherwise collide once there's more than one segment.

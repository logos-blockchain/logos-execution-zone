...
//! a native Transfer (minutes, runs once outside the timed loop). Measured
...
use cycle_bench::ppe::prove_native_transfer_in_ppe;
use lee::PRIVACY_PRESERVING_CIRCUIT_ID;
use risc0_zkvm::{InnerReceipt, Receipt};

fn bench_verify(c: &mut Criterion) {
    let (output, proof) = prove_native_transfer_in_ppe().expect("prove native transfer in PPE");
    ...
    g.bench_function("verify_native_transfer", |b| {
        b.iter(|| {
            receipt

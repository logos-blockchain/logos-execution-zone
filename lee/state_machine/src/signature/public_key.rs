...
impl FromStr for PublicKey {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        hex::decode_to_slice(s, &mut bytes).map_err(LeeError::InvalidHexPublicKey)?;
        Self::try_new(bytes)
    }
}
...
impl PublicKey {
    ...
    pub fn try_new(value: [u8; 32]) -> Result<Self, LeeError> {
        ...
        let _ =
            k256::schnorr::VerifyingKey::from_bytes(&value).map_err(LeeError::InvalidPublicKey)?;
...
#[cfg(test)]
mod test {
    use crate::{PublicKey, error::LeeError, signature::bip340_test_vectors};
    ...
    fn try_new_invalid_public_key_from_bip340_test_vectors_5() {
        ...
        assert!(matches!(result, Err(LeeError::InvalidPublicKey(_))));
    }

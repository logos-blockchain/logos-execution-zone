...
fn transition_from_authenticated_transfer_program_invocation_default_account_destination() {
    ...
    assert_eq!(state.get_account_by_id(from).balance(native()), 95);
    assert_eq!(state.get_account_by_id(to).balance(native()), 5);
...
fn transition_from_authenticated_transfer_program_invocation_insuficient_balance() {
    ...
    assert!(state.get_account_by_id(from).balance(native()) < balance_to_move);
    ...
    assert_eq!(state.get_account_by_id(from).balance(native()), 100);
    assert_eq!(state.get_account_by_id(to).balance(native()), 0);
...
fn transition_from_authenticated_transfer_program_invocation_non_default_account_destination() {
    ...
    let mut state = V03State::new()
        .with_public_account_balances(native(), [(account_id1, 100), (account_id2, 200)])
        .with_test_programs();
    ...
    assert_eq!(state.get_account_by_id(from).balance(native()), 192);
    assert_eq!(state.get_account_by_id(to).balance(native()), 108);
    assert_eq!(state.get_account_by_id(from).nonce, Nonce(1));
    assert_eq!(state.get_account_by_id(to).nonce, Nonce(1));
}
...
fn transition_from_sequence_of_authenticated_transfer_program_invocations() {
    ...
    let account_id2 = AccountId::from(&PublicKey::new_from_private_key(&key2));
    let mut state = V03State::new()
        .with_public_account_balances(native(), [(account_id1, 100)])
        .with_test_programs();
    ...
    assert_eq!(state.get_account_by_id(account_id1).balance(native()), 95);
    assert_eq!(state.get_account_by_id(account_id2).balance(native()), 2);
    assert_eq!(state.get_account_by_id(account_id3).balance(native()), 3);

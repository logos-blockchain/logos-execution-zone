...
fn program_should_fail_if_it_drops_a_declared_account() {
    ...
    let message = public_transaction::Message::try_new(program_id, positions, vec![], ()).unwrap();
...
fn program_should_fail_if_it_debits_an_unauthorized_account() {
    ...
    let message = public_transaction::Message::try_new(
        ...
        vec![
            ...
            Position::balance_only(receiver_account_id),
        ],
        vec![],
        balance_to_move,
    )
...
/// Rule 3: data may only be written on a position naming the executing program, so a position
/// handed to `foreign_shard_writer` under someone else's namespace is unwritable.
#[test]
fn a_data_write_on_a_foreign_namespace_is_rejected_publicly() {
    ...
    let foreign_namespace: AccountId = crate::test_methods::data_changer().id().into();
...
/// The same guest on its own namespace: the shard lands there and nowhere else.
#[test]
fn a_data_write_on_the_executing_namespace_is_accepted_publicly() {
    let target_id = AccountId::new([1; 32]);
    let other_id = AccountId::new([2; 32]);
    let mut state = V03State::new().with_test_programs();
    let program_id: AccountId = crate::test_methods::foreign_shard_writer().id().into();
    ...
    let message = public_transaction::Message::try_new(
        program_id,
        vec![
            Position::new(target_id, program_id),
            Position::balance_only(other_id),
        ],
        vec![],
        written.clone(),
...
/// account in its own output — one it was never given via `ChainedCall.positions`. This must be
...
/// A callee must account for exactly the positions its caller named. `dropped_account` is handed

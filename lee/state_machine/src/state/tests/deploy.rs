...
fn manually_segmented_program_reconstructs_and_executes_identically() {
    ...
    let pre_states = vec![Input::balance_only(AccountId::new([21; 32]), true, 0)];
...
fn program_with_more_than_max_segments_is_rejected_at_deploy_time() {
    ...
    let mut positions = vec![Position::new(header_account_id, PROGRAM_LOADER_ACCOUNT_ID)];
    positions.extend(
        segment_account_ids
            .iter()
            .map(|id| Position::new(*id, PROGRAM_LOADER_ACCOUNT_ID)),
    );
...
fn write_segment_then_create_header_deploys_a_dispatchable_program() {
    ...
    let write_segment_message = public_transaction::Message::try_new(
        PROGRAM_LOADER_ACCOUNT_ID,
        vec![Position::new(segment_account_id, PROGRAM_LOADER_ACCOUNT_ID)],
        vec![Nonce(0)],
    ...
    let create_header_message = public_transaction::Message::try_new(
        PROGRAM_LOADER_ACCOUNT_ID,
        vec![
            Position::new(header_account_id, PROGRAM_LOADER_ACCOUNT_ID),
            Position::new(segment_account_id, PROGRAM_LOADER_ACCOUNT_ID),
        ],
        vec![Nonce(0)],
        Instruction::CreateHeader {
    ...
    let call_message = public_transaction::Message::try_new(
        header_account_id,
        vec![Position::balance_only(target_id)],
        vec![],
        (),
    )

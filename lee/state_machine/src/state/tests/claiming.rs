...
fn separate_initialize_and_fund_chain_calls_allow_unauthorized_private_recipient() {
    // Two separate chained calls: one to `claimer` (initializes the recipient via
    // `Claim::Authorized`, zero balance change), then one to `simple_balance_transfer` (funds
    // it from the sender; the recipient is no longer default by then, so no second claim
    // happens there). The recipient is private and `is_authorized` is left `false` throughout,
    // with no witnessed `ask` at all. Expected to succeed regardless — private accounts
    // intentionally skip claim-authorization entirely (see the `Claim::Authorized => {}` no-op
    // in `execution_state.rs`'s materialization for private accounts), since a private
    // account's real security lives in the commitment/nullifier scheme, not in this
    // bookkeeping.
    ...
    let (output, proof) = execute_and_prove(
        // `initialize_then_fund` echoes its own `pre_states` back in the same order it
        // received them, so `account_identities` below matches straightforwardly.
        vec![
    ...
    // Private accounts aren't stored in the public `AccountId -> Account` map `get_account_by_id`
    // reads from — their state lives as a commitment, checked here by reconstructing the exact
    // final account and confirming a matching commitment was actually added to state.
    let expected_recipient_post = Account {
...
fn separate_initialize_and_fund_chain_calls_succeed_publicly_for_public_recipient() {
    // Sanity baseline for
    // `separate_initialize_and_fund_chain_calls_fail_privately_for_public_recipient` below: the
    // same two-separate-chained-calls shape, but with a real signer-keyed public
    // recipient (matching `claiming_mechanism_within_chain_call`'s recipient, just split across
    // two calls instead of one combined `simple_balance_transfer` call) and executed publicly.
    let initializer = crate::test_methods::initialize_then_fund();
    let claimer = crate::test_methods::claimer();
    let claimer_id = claimer.id();
    let simple_transfer = crate::test_methods::simple_balance_transfer();
...
// `separate_initialize_and_fund_chain_calls_reject_stale_pre_state_publicly`/`_privately`
// previously lived here, proving that `initialize_then_fund` couldn't hand the fund call a
// stale (pre-claim) recipient pre-state. Since `ChainedCall.pre_state_refs` no longer carries
// an `Account` value at all — the calling program only names accounts by id, and the protocol
// resolves each one's real current state — there's no longer a channel for a program to supply
// a stale value in the first place. That failure mode is a compile-time impossibility now, not
// something a runtime check needs to keep catching. See the equivalent note in
// `validated_state_diff/tests.rs`.

#[test]
fn separate_initialize_and_fund_chain_calls_for_public_recipient_privately() {
    // Same scenario as
    // `separate_initialize_and_fund_chain_calls_succeed_publicly_for_public_recipient`
    // (public recipient, real signer, two separate chained calls claiming then funding it),
    // executed through the privacy-preserving circuit instead. Checks whether splitting the
    // claim and the funding into two separate `AccountDiff`-carrying calls on the same public
    // account within one transaction is handled correctly there too.
    let initializer = crate::test_methods::initialize_then_fund();
    let claimer = crate::test_methods::claimer();
    let claimer_id = claimer.id();
    let simple_transfer = crate::test_methods::simple_balance_transfer();
    ...
    // untouched by any chained call and exists purely to satisfy that, not as part of the
    // claim-and-fund scenario under test.

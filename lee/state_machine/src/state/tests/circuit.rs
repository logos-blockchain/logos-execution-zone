...
/// anywhere and is dispatched as a shadow program instead — its identity is established fresh,
/// in this one proof, from its elf supplied as a witness. Confirms the shadow-resolved dispatch
/// address flows into the same `Claim::Pda` ownership mechanics every other program uses, and
/// that a shadow program never appears in the circuit's `program_image_claims` output.
#[test]
...
/// A shadow-dispatched program can claim a *public* PDA too — nothing restricts shadow programs
/// to private accounts. Ownership stays gated the same self-authenticating way either way:
/// reproducing the dispatch address (and later acting as its owner) always requires re-supplying
/// the exact same elf as a witness, whether the account being claimed is public or private.
#[test]
fn shadow_program_claims_a_public_pda_it_legitimately_owns() {
    let program = crate::test_methods::pda_claimer();
    let seed = PdaSeed::new([42; 32]);
    let shadow_account_id = AccountId::for_shadow_program(&program.id());
    let pda_account_id = AccountId::for_public_pda(&shadow_account_id, &seed);
    let pre_state = AccountWithMetadata::new(Account::default(), false, pda_account_id);
    ...
    let (output, _proof) = result.expect("a shadow program's public PDA claim should succeed");
...
fn sibling_declaring_delegated_pda(pda_is_authorized: bool) -> Result<(), LeeError> {
    ...
    let callee_id = callee.deployed_account_id();
    let sibling_id = sibling.deployed_account_id();
...
fn delegated_pda_stays_authorized_in_delegated_subtree() {
    ...
    let forwarder_id = forwarder.deployed_account_id();
    let callee_id = callee.deployed_account_id();
    let program_with_deps = ProgramWithDependencies::new(
        delegator,
        [(forwarder_id, forwarder), (callee_id, callee)].into(),
    );
    let no_sibling: Option<(AccountId, Option<bool>)> = None;
...
fn undeclaring_private_delegation(
    ...
    let delegator_id = delegator.deployed_account_id();
...
fn undeclaring_public_delegation(
    ...
    let pre_state = AccountWithMetadata::new(
        Account {
            program_owner: delegator.deployed_account_id(),
            ..Account::default()
        },
        false,
        account_id,
...
fn delegated_public_pda_first_seen_in_callee_is_authorized() {
    let seed = PdaSeed::new([77; 32]);
    let delegator_id = crate::test_methods::undeclaring_pda_delegator().deployed_account_id();
...
fn private_account_claimed_then_used_without_init_flag_should_fail() {
    ...
    let account_metadata = {
        ...
        acc.account.program_owner = crate::test_methods::claimer().deployed_account_id();
        acc
    };
...
fn private_claim_matching_a_real_commitment_passes_verification() {
    ...
    let program_with_deps = ProgramWithDependencies::from(program)
        .with_program_account_id(header_account_id)

...
fn public_echo_of_a_default_account_leaves_it_unowned() {
    ...
    let program_id = AccountId::from_builtin_program(crate::test_methods::noop().id());
...
fn a_squatter_acquires_a_funded_account_but_still_cannot_spend_it() {
    ...
    let program_id = AccountId::from_builtin_program(crate::test_methods::squatter().id());
...
fn a_private_write_to_a_foreign_owned_account_is_rejected() {
    let program = crate::test_methods::data_changer();
    let owner_id = AccountId::from_builtin_program(crate::test_methods::noop().id());
    let sender_keys = test_private_account_keys_1();
    let owned = Account {
...
fn private_credit_to_a_public_unowned_recipient_leaves_it_unowned() {
    ...
    state.register_program(&program);
...
fn a_callee_cannot_write_an_account_the_caller_acquired_in_the_same_transaction() {
    let mut state = V03State::new().with_test_programs();
    let caller_id =
        AccountId::from_builtin_program(crate::test_methods::acquire_and_forward().id());
...
fn acquire_then_fund_chain_calls_allow_unauthorized_private_recipient() {
    ...
    let acquirer_id = AccountId::from_builtin_program(acquirer.id());
    let transfer = crate::test_methods::simple_balance_transfer();
    let transfer_id = AccountId::from_builtin_program(transfer.id());
...
fn acquire_then_fund_chain_calls_succeed_publicly_for_public_recipient() {
    let program = crate::test_methods::acquire_then_fund();
    let acquirer_id = AccountId::from_builtin_program(crate::test_methods::data_changer().id());
    let transfer_id =
        AccountId::from_builtin_program(crate::test_methods::simple_balance_transfer().id());

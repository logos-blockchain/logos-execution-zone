...
impl ProgramCommitmentDigest {
    ...
    /// Queries the `ProgramCommitmentDigest` for a membership proof of a commitment.
...
impl V03State {
    ...
    /// Inserts a program and appends its `ProgramCommitment` to the `ProgramCommitmentDigest` in
    /// the same step. Every program that ever enters `programs` — genesis-seeded via
    /// `with_programs`, or deployed via `transition_from_program_deployment_transaction` —
    /// goes through this one function, so the commitment digest can never drift out of sync
    /// with the program registry itself.
    pub(crate) fn insert_program(&mut self, program: Program) {
        let commitment = ProgramCommitment::for_program_id(program.id());

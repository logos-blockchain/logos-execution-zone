...
impl V03State {
    ...
    /// `user_elf`, chunked the same way a live `program_loader` deploy would — just written
    /// directly rather than through a transaction. The header keeps living at its builtin default
    /// address (`AccountId::from_builtin_program(program.id())`) so existing call sites
    /// addressing builtins by `ProgramId` keep working; each segment's address is this function's
    /// own internal convention, never independently recomputed elsewhere.
    ///
    /// Also used by tests: any test running a privacy-preserving transaction needs its programs
    /// registered here first, since `check_privacy_preserving_circuit_proof_is_valid` claims them
    /// against real chain state, not just the local prover.

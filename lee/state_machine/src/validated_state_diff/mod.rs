...
impl ValidatedStateDiff {
    pub fn from_public_transaction(
        ...
        while let Some((chained_call, caller_data)) = chained_calls.pop_front() {
            ...
            {
                // The loader runs as native Rust instead of interpreting a guest ELF — there's no
                // guest binary for it.
                ...
                // Every loader instruction validates its input via `assert!`/`.expect(...)`,
                // exactly like every guest program in this codebase, relying here on

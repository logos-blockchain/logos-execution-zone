...
struct CallerData {
    ...
    /// `V03State::get_program`) rather than guessed from `account_id` via the bijection — needed
    /// wherever PDA derivation requires the caller's actual identity, since a `Deploy`-created
    /// caller's address doesn't encode it.

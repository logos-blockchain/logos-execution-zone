...
use crate::{
    ...
    validated_state_diff::{ValidatedStateDiff, public_backend::PublicBackend},
};
...
/// A `PublicBackend` sized for a single `resolve_public_action` call in these tests - `block_id`
/// and `timestamp` are unused by `resolve_write`, so any value works.
fn backend_for(state: &V03State) -> PublicBackend<'_> {
    PublicBackend::new(
        state,
        1,
        0,
...
/// settlement-time counterpart to `resolve_write`'s own role in `execute_authorized`.
#[test]
...
fn resolve_public_action_fails_when_live_balance_cannot_cover_a_stale_deferred_debit() {
    ...
    let mut backend = backend_for(&state);
    let result = super::resolve_public_action(&action, &state, &mut backend);
...
/// `resolve_public_action`'s own addition beyond the original design: an account's real
/// ownership can differ by settlement time from what the prover saw at proof time, so a
/// `Deferred` resolution claiming a program that doesn't actually own the account (and the
/// account isn't unowned either) must be rejected, not silently applied.
#[test]
fn resolve_public_action_rejects_a_write_from_a_program_that_does_not_own_the_account() {
    let owner_program_id: AccountId = crate::test_methods::stripped_token().id().into();
    let intruder_program_id: AccountId = crate::test_methods::simple_balance_transfer().id().into();
    let account_id = AccountId::new([9; 32]);

    let mut state = V03State::new().with_test_programs();
    initialize_stripped_token_account(&mut state, owner_program_id, account_id, 100, 1);

    // `simple_balance_transfer` never implemented `Incremental`, so this resolves verbatim,
    // never actually reaching the account's real data - the check must catch it before that.
    let action = PublicActionWithID::Deferred {
        account_id,
        resolutions: vec![DeferredResolution {
            executing_account_id: intruder_program_id,
            post_balance_diff: BalanceDiff::Add(0),
            post_data: Some(vec![9, 9, 9].try_into().unwrap()),
        }],
    };
    let mut backend = backend_for(&state);
    let result = super::resolve_public_action(&action, &state, &mut backend);
    ...
    assert!(
        ...
        "expected UnauthorizedDataModification, got {result:?}"
    );
}

/// A `Bound` action passes through unchanged — regression guard against `resolve_public_action`
/// disturbing today's only real code path.
#[test]

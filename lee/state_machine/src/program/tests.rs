...
use lee_core::{
    account::{AccountId, BalanceDiff, Input},
...
fn transfer_fixture() -> (Program, Vec<Input>, Vec<u8>, u128) {
    let program = crate::test_methods::simple_balance_transfer();
    let balance_to_move: u128 = 11_223_344_556_677;
    let instruction_data = Program::serialize_instruction(balance_to_move).unwrap();
    let sender = Input::balance_only(AccountId::new([0; 32]), true, 77_665_544_332_211);
    let recipient = Input::balance_only(AccountId::new([1; 32]), false, 0);
    (
        program,
...
fn journal_is_the_borsh_frame_of_the_output_and_echoes_instruction_data() {
    ...
    let pre_states = [
        Input::balance_only(AccountId::new([0; 32]), true, 10),
        Input::balance_only(AccountId::new([1; 32]), false, 0),
    ];

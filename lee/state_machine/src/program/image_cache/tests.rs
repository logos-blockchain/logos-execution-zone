...
use lee_core::{
    account::{AccountId, Cycles, Data, Input},
...
fn transfer_pre_states() -> Vec<Input> {
    vec![
        Input::balance_only(AccountId::new([0; 32]), true, 77_665_544_332_211),
...
fn cached_path_matches_rebuild_path() {
    let cases: Vec<(&str, Program, Vec<Input>, Vec<u8>)> = vec![
        (

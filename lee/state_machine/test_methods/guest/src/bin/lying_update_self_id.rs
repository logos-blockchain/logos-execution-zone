...
/// An identity `Incremental` program, but its `Update` handler reports the wrong
/// `self_account_id` — exercises `resolve_write`'s check that an `Update` resolution was
/// actually produced by the program it claims to be, not a different one.
fn main() {
    ...
    match call {
        ...
        ProgramCall::Update(ProgramInput {
            ...
            ProgramOutput::new(
                DEFAULT_PROGRAM_ID.into(), // WRONG: should be the real self_account_id
                None,
                instruction_data,
                vec![diff],
            )

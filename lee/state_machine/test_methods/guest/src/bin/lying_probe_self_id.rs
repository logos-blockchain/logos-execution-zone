...
/// Implements `Incremental`, but its `Probe` handler reports the wrong `self_account_id` —
/// exercises `verify_probe_receipt`'s check that a `Probe` receipt was actually produced by the
/// program it claims to answer for.
fn main() {
    ...
    match call {
        ...
        ProgramCall::Probe(input) => {
            ...
            ProgramOutput::new(
                DEFAULT_PROGRAM_ID.into(), // WRONG: should be input.self_account_id

...
fn main() {
    ...
    // A delegated seed authorizes the sender for the callee, as `pda_seeds` below.
    if pda_seed.is_some() {
    ...
    for _i in 0..num_chain_calls {
        ...
        let mut sender_slot = running_sender_pre.slot_of(simple_transfer_id).clone();
        sender_slot.balance = match sender_slot.balance.checked_sub(balance) {
            Some(new_balance) => new_balance,
            None => return,
        };
        running_sender_pre = running_sender_pre.with_slot(simple_transfer_id, sender_slot);

        let mut recipient_slot = running_recipient_pre.slot_of(simple_transfer_id).clone();
        recipient_slot.balance = match recipient_slot.balance.checked_add(balance) {
            Some(new_balance) => new_balance,
            None => return,
        };
        running_recipient_pre = running_recipient_pre.with_slot(simple_transfer_id, recipient_slot);
    }

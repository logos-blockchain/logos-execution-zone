...
/// The `AccountId` in the instruction must be the dispatch address of the transfers program.
fn main() {
    let (
        ProgramInput {
            self_account_id,
            caller_account_id,
            pre_states,
            instruction: (balance, simple_transfer_account_id, num_chain_calls, pda_seed),
        },
        instruction_words,
    ...
    for _i in 0..num_chain_calls {
        let new_chained_call = ChainedCall {
            program_account_id: simple_transfer_account_id,

...
/// prefixed length (e.g. transport word-alignment padding). Returns `None` if `bytes` is shorter
/// than the 4-byte length prefix or the prefixed length exceeds the available payload; callers
/// decoding untrusted journals must treat `None` as a rejection rather than panicking.
#[must_use]

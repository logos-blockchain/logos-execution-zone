...
pub fn read_lee_call<T: BorshDeserialize>() -> ProgramCall<T> {
    ...
    dispatch_call(call_kind, envelope)
}

/// The decode-and-dispatch half of [`read_lee_call`], taking its inputs as plain values instead
/// of reading them from the guest environment - so this can be exercised directly, without a
/// zkVM execution.
fn dispatch_call<T: BorshDeserialize>(
    call_kind: CallKind,
    envelope: ProgramInput<InstructionData>,
) -> ProgramCall<T> {
    match call_kind {
        CallKind::Execute => {
            let ProgramInput {

...
fn shard_state_diff_constructors() {
    ...
    let pre = Input::named(AccountId::new([7; 32]), true, 5, program, pre_data.clone());
    let post_data: Data = vec![0xde, 0xad, 0xbe, 0xef].try_into().unwrap();

    let unchanged = ShardStateDiff::unchanged(pre.clone());
    assert_eq!(unchanged.post_balance_diff, BalanceDiff::Add(0));
    assert_eq!(unchanged.post_data, None);

    let balance_only = ShardStateDiff::balance_only(pre.clone(), BalanceDiff::Sub(2));
    assert_eq!(balance_only.post_balance_diff, BalanceDiff::Sub(2));
    assert_eq!(balance_only.post_data, None);

    // `new` never elides data equal to the pre-state's.
    let written = ShardStateDiff::new(pre.clone(), BalanceDiff::Add(1337), pre_data.clone());
    assert_eq!(written.pre, pre);
    assert_eq!(written.post_balance_diff, BalanceDiff::Add(1337));
    assert_eq!(written.post_data, Some(pre_data));

    assert_eq!(
        ShardStateDiff::new(pre, BalanceDiff::Add(0), post_data.clone()).post_data,
        Some(post_data)
    );
}
...
fn validate_execution_rejects_insufficient_balance_even_if_globally_conserved() {
    ...
    let pre = Input::named(account_id, true, 5, executing_program_id, Data::empty());
    let state_diffs = [ShardStateDiff::balance_only(pre, BalanceDiff::Sub(10))];
...
fn validate_execution_rejects_add_overflow() {
    ...
    let state_diffs = [ShardStateDiff::balance_only(pre, BalanceDiff::Add(1))];
...
#[test]
fn a_data_write_on_a_foreign_namespace_is_rejected() {
    let executing_account_id = AccountId::new([2; 32]);
    let account_id = AccountId::new([7; 32]);
    let pre = Input::named(account_id, true, 5, AccountId::new([1; 32]), Data::empty());
...
#[test]
fn a_data_write_on_the_executing_namespace_is_accepted() {
    let executing_account_id = AccountId::new([2; 32]);
    let pre = Input::named(
        AccountId::new([7; 32]),
        true,
...
#[test]
fn a_balance_only_position_cannot_carry_data() {
    let executing_account_id = AccountId::new([2; 32]);
    let account_id = AccountId::new([7; 32]);
    let pre = Input::balance_only(account_id, true, 5);
    let state_diffs = [ShardStateDiff::new(
        pre,
        BalanceDiff::Add(0),
        b"record".to_vec().try_into().unwrap(),
...
#[test]
fn two_positions_of_one_account_in_a_call_are_rejected() {
    let executing_account_id = AccountId::new([2; 32]);
    let account_id = AccountId::new([7; 32]);
    let state_diffs = [
        ShardStateDiff::unchanged(Input::named(
            account_id,
            true,
            5,
            executing_account_id,
        ...
        ShardStateDiff::unchanged(Input::balance_only(account_id, true, 5)),
    ];
    ...
    assert!(matches!(
        result,
        Err(ExecutionValidationError::DuplicatePosition { account_id: id }) if id == account_id
    ));
}
...
fn get_program_via_reads_the_loader_shard() {
    ...
    let lookup = |id| {
        if id == program_account {
            shard(PROGRAM_LOADER_ACCOUNT_ID, header.to_bytes())
        } else if id == segment_account {
            shard(PROGRAM_LOADER_ACCOUNT_ID, segment.to_bytes())
        } else {
            Account::default()
        }
    };

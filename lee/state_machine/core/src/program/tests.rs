...
fn for_private_pda_matches_pinned_value() {
    ...
    let identifier: Identifier = (u128::MAX, u128::MAX);
...
fn for_private_pda_differs_for_different_npk() {
    ...
    let vpk = ViewingPublicKey::from_seed(&[1_u8; 32], &[2_u8; 32]);
    assert_ne!(
        AccountId::for_private_pda(&program_id, &seed, &npk_a, &vpk, (u128::MAX, u128::MAX)),
        AccountId::for_private_pda(&program_id, &seed, &npk_b, &vpk, (u128::MAX, u128::MAX)),
    );
}
...
fn for_private_pda_differs_for_different_seed() {
    ...
    let vpk = ViewingPublicKey::from_seed(&[1_u8; 32], &[2_u8; 32]);
    assert_ne!(
        AccountId::for_private_pda(&program_id, &seed_a, &npk, &vpk, (u128::MAX, u128::MAX)),
        AccountId::for_private_pda(&program_id, &seed_b, &npk, &vpk, (u128::MAX, u128::MAX)),
    );
}
...
fn for_private_pda_differs_for_different_program_id() {
    ...
    let vpk = ViewingPublicKey::from_seed(&[1_u8; 32], &[2_u8; 32]);
    assert_ne!(
        AccountId::for_private_pda(&program_id_a, &seed, &npk, &vpk, (u128::MAX, u128::MAX)),
        AccountId::for_private_pda(&program_id_b, &seed, &npk, &vpk, (u128::MAX, u128::MAX)),
    );
}
...
fn for_private_pda_differs_for_different_identifier() {
    ...
    let vpk = ViewingPublicKey::from_seed(&[1_u8; 32], &[2_u8; 32]);
    assert_ne!(
        AccountId::for_private_pda(&program_id, &seed, &npk, &vpk, (0, 0)),
        AccountId::for_private_pda(&program_id, &seed, &npk, &vpk, (0, 1)),
    );
    assert_ne!(
        AccountId::for_private_pda(&program_id, &seed, &npk, &vpk, (0, 0)),
        AccountId::for_private_pda(&program_id, &seed, &npk, &vpk, (u128::MAX, u128::MAX)),
    );
}
...
fn for_private_pda_differs_from_public_pda() {
    ...
    let npk = NullifierPublicKey([3; 32]);
    let vpk = ViewingPublicKey::from_seed(&[1_u8; 32], &[2_u8; 32]);
    let private_id = AccountId::for_private_pda(&program_id, &seed, &npk, &vpk, (u128::MAX, u128::MAX));
...
fn private_account_kind_header_round_trips() {
    let regular = PrivateAccountKind::Regular((0, 42));
    let pda = PrivateAccountKind::Pda {
        account_id: AccountId::new([1; 32]),
        seed: PdaSeed::new([2_u8; 32]),
        identifier: (u128::MAX, u128::MAX),
    };
    assert_eq!(

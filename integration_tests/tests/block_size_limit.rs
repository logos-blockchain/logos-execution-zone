...
async fn transaction_deferred_to_next_block_when_current_full() -> Result<()> {
    let simple_transfer = test_programs::simple_balance_transfer();
    ...
    // (e.g., 10 KiB)
    let max_program_size = simple_transfer.elf().len().max(chain_caller.elf().len());
    ...
    ctx.sequencer_client()
        .send_transaction(LeeTransaction::ProgramDeployment(
            lee::ProgramDeploymentTransaction::new(
                lee::program_deployment_transaction::Message::new(simple_transfer.elf().to_owned()),
            ),
        ))
        .await?;
    ...
    assert_eq!(
        ...
        simple_transfer.id(),
        "Expected simple_balance_transfer program to be deployed in block 1"
    );

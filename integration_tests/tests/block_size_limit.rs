...
async fn accept_transaction_within_limit() -> Result<()> {
    ...
    // segment under the current cap. `send_transaction`'s Ok/Err here reflects size and
    // signature-shape checks only, not authorization, which is only checked at actual state
    // execution — but a fee declaration and its payer's signature are still required up front, so
    // this uses a real (funded) payer key rather than going unsigned.
    let bytecode =
        test_programs::claimer().elf()[..program_loader_core::MAX_SEGMENT_DATA_LEN].to_vec();
    let payer_data = &testnet_initial_state::initial_pub_accounts_private_keys()[0];
    let message = lee::public_transaction::Message::try_new_with_fees(
        PROGRAM_LOADER_ACCOUNT_ID,
        vec![AccountId::new([2; 32])],
        // One nonce for the payer, the transaction's only witness signer.
        vec![lee_core::account::Nonce(0)],
        program_loader_core::Instruction::WriteSegment {
            bytecode,
            next_segment: None,
        },
        common::test_utils::test_fee_declaration(payer_data.account_id),
    )?;
    let witness_set =
        lee::public_transaction::WitnessSet::for_message(&message, &[&payer_data.pub_sign_key]);
    let tx = LeeTransaction::Public(lee::PublicTransaction::new(message, witness_set));
    let tx_size = encoded_tx_size(&tx);
...
async fn transaction_deferred_to_next_block_when_current_full() -> Result<()> {
    ...
    // Both deploys share one fee payer, so its nonce must be threaded across them in submission
    // order — see `deploy_program_transactions`'s `payer_nonce`.
    let mut payer_nonce: u128 = 0;

    let claimer_key = PrivateKey::try_new([3; 32]).unwrap();
    let (_claimer_header, claimer_txs) =
        deploy_program_transactions(claimer.elf(), 10, &claimer_key, &mut payer_nonce);
    ...
    let (_chain_caller_header, chain_caller_txs) =
        deploy_program_transactions(chain_caller.elf(), 40, &chain_caller_key, &mut payer_nonce);

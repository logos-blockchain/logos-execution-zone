...
use test_fixtures::{BlockingTestContext, config::bedrock_channel_id};
...
fn sequencer_ffi_join_setup_and_simple_queries_test() -> Result<()> {
    ...
    wait_for_joining_sequencer_to_produce_block(sequencer_ffi, &ctx, &node)?;
...
fn sequencer_ffi_check_status_correctness() -> Result<()> {
    ...
    // SAFETY: produced by FFI, is valid.
    unsafe {
        sequencer_ffi_helpers::sequencer_ffi_free_ffi_sequencer_status(ffi_status_res.value);
    }
...
fn wait_for_joining_sequencer_to_produce_block(
    sequencer_ffi: &SequencerServiceFFI,

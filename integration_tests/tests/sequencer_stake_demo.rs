...
async fn stake_transaction_joins_the_bedrock_committee() -> Result<()> {
    ...
    ctx.wallet()
        .send_pub_tx(
            vec![
                ...
                AccountIdentity::Public(ownership_id).in_namespace(stake_id),
                AccountIdentity::PublicNoSign(funds_id).balance_only(),
                AccountIdentity::PublicNoSign(config_id).in_namespace(stake_id),
            ],
    ...
    assert!(
        !ownership_account.shard(stake_id).is_empty(),
    ...
    let record =
        sequencer_stake_core::StakeRecord::from_bytes(ownership_account.shard(stake_id).as_ref())
    ...
    let drained_record = sequencer_stake_core::StakeRecord::from_bytes(
        drained_ownership_account.shard(stake_id).as_ref(),
    )

...
async fn private_pda_family_members_receive_and_spend() -> Result<()> {
    ...
    // Unlike genesis-seeded `auth_transfer`, `pda_spend_proxy` is deployed fresh below, so its
    // header target must be a real key the wallet signs for — `program_loader` requires
    // `is_authorized` for `CreateHeader`.

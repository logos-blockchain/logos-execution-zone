...
use lee::{
    AccountId, Position, PrivacyPreservingTransaction, PrivateKey, ProgramId, ProvingInput,
    PublicKey,
...
use lee_core::{
    DUMMY_COMMITMENT_HASH, NullifierPublicKey, NullifierWitness, PrivateWitness, WitnessKind,
    account::Account, encryption::ViewingPublicKey, program::PdaSeed,
};
...
async fn fund_private_pda(
    ...
    let (output, proof) = execute_and_prove(
        ProvingInput {
            positions: vec![
                Position::balance_only(sender),
                Position::balance_only(pda_account_id),
            ],
            signers: [sender].into(),
        ...
        .send_privacy_preserving_tx(
            vec![
                AccountIdentity::PrivatePdaOwned {
                    ...
                    authority: spend_program.self_account_id,
                    seed,
                }
                .balance_only(),
...
async fn private_pda_family_members_receive_and_spend() -> Result<()> {
    ...
    let header_message = lee::public_transaction::Message::try_new_with_fees(
        lee_core::program::PROGRAM_LOADER_ACCOUNT_ID,
        vec![
            Position::new(proxy_id, lee_core::program::PROGRAM_LOADER_ACCOUNT_ID),

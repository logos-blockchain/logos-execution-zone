...
const MINT_SRC_ACCOUNT_ID: AccountId = AccountId::new([9; 32]);
const DECOY_ACCOUNT_ID_A: AccountId = AccountId::new([3; 32]);
const DECOY_ACCOUNT_ID_B: AccountId = AccountId::new([4; 32]);
...
fn seed_ping_sender_config(state: &mut V03State) {
    ...
    *state = std::mem::replace(state, V03State::new()).with_public_accounts([(
        ...
        Account {
            ...
            data: outbox_bytes(AccountId::from_builtin_program(
...
fn source_minted(state: &V03State) -> u128 {
    let config_id = wrapped_token_core::config_account_id(AccountId::from_builtin_program(
    ...
    let mut entries: Vec<_> = (0..16_u8)
        .map(|index| wrapped_token_core::SourceEntry {
            policy: wrapped_token_core::SourcePolicy {
                ...
                src_account_id: AccountId::from_builtin_program([u32::from(index) + 100; 8]),
...
fn a_lock_naming_another_target_program_is_rejected() {
    ...
    let elsewhere = AccountId::from_builtin_program(programs::ping_receiver().id());
    let lock = lock_tx_to(
        &holder_key,
        holder_id,
        zone_b,
...
fn the_governance_path_guards_hold() {
    ...
    seed_wrapped_config_with_governance(
        &mut other,
        Some(AccountId::from_builtin_program(
...
fn a_top_level_mint_is_refused() {
    ...
    let wrapped_token_id = AccountId::from_builtin_program(programs::wrapped_token().id());
    let src_zone = [2_u8; 32];
    let src_program_id = AccountId::from_builtin_program(programs::bridge_lock().id());

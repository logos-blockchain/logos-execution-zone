...
/// A `SupplyAccount` genesis action for a fresh account, returning the account id the
/// funds land in, so tests can assert genesis state is present.
...
async fn empty_local_and_empty_bedrock_bootstraps_from_genesis() -> Result<()> {
    ...
    assert_eq!(
        client
            .get_account_balance(supplied_id, programs::authenticated_transfer().id())
            .await?,
        12_345,
